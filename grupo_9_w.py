...
def actualizar_reloj():
    hora_actual = time.strftime("%H:%M:%S")
    etiqueta_reloj.config(text=hora_actual)
    # Actualiza la etiqueta cada 1000 ms (1 segundo)
    etiqueta_reloj.after(1000, actualizar_reloj)
...
root.configure(bg="#89F97A")  # Color verde claro para la ventana principal

#imagen_fondo = Image.open("//imagen_info.png")
...
etiqueta_reloj = tk.Label(root, font=("Arial", 48), bg="black", fg="white")
etiqueta_reloj.pack(anchor="center", expand=True)

# Iniciar la actualización del reloj
actualizar_reloj()# armado del menú desplegable
...
integrantes = ["Noemi Ciscka", "Burgos Mauricio", "Esteban Ayala", "Yamila", "Wanda Sabadini"]
